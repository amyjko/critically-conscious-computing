...
Thousands of researchers, and eventually millions of engineers invented answers to these questions over decades, eventually resulting in foundational computer networking ideas like packet switching, transmission control protocol (TCP), internet protocol (IP), hypertext transfer protocol (HTTP), the worldwide web, and browsers. These are the ideas that now power the modern Internet, implementing the design that Licklider had envisioned.
...
In design, the mindset is one that centers imagination, possibility, ideation, human values, ethics, humanity, politics, society, and justice<costanzachock20>. Design mindsets are not concerned with whether something is possible or how it might be possible, but rather what should be, and how that might be better from what is now<buxton10>. Thinking like a designer requires forgetting how the world works now to make space for imagining how it might work better, but also deeply engaging with how the world works now to understand how it might be improved, and whether design ideas would actually improve it.

Design _methods_ are how designers do design<goodwin11>. Methods include techniques for understanding problems in the world, speculating about ways to address them, sketching and prototyping solutions, soliciting feedback, all while engaging with the stakeholders that might be affected if a design idea were to eventually be engineered. Methods are often based on rigorous theory about how people and technology interact<carroll03>.

While Licklider was approaching the problem of networking computers with a design mindset, imagining a future that did not yet exist, his methods were highly exclusionary, only involving the U.S. Department of Defense and a handful of white male professors at elite universities. This is despite the fact that he was fully aware of the risk that computing might only reach a favored few:
...
Inclusive design methods also focus on helping designers see past their assumptions. For example, one method called *assumption ideation* involves analyzing how software excludes and does harm. To do this ideation, one might consider a particular software service, and brainstorm all of the different people, identities, abilities, and contexts in which someone might use it. If this brainstorming is broad, examining a diversity of intersectional identities and cultures, the exclusion and harm becomes more apparent. For example, let’s consider the computer mouse, a ubiquitous input device. Who are all of the different people who might use it, and what assumptions does a mouse make that might influence how they use it? We might start off with people with disabilities. For example, can a person who is blind use a mouse? No, they cannot track the mouse cursor on a screen. Can a person who has severe motor tremors due to multiple sclerosis use a mouse? Not really, since most mice and operating systems require precise motor movements. Can a person who does not have hands use a mouse? Obviously not. Even this simple ideation reveals that the computer mouse is intrinsically exclusionary, really only working for people with sight and fine motor control over a hand. A designer that prioritizes disability justice would have rejected the idea of a mouse when Douglas Englebart first invented it in 1961<myers98>.
...
Engineering _methods_ are ones that involve invention, specification, formalization, testing, proving, analyzing, implementation, and constraint satisfaction. That is not to say that engineering does not involve design methods of its own ways -- many of these activities require a kind of prototyping and iteration of technical solutions -- but _engineering_ design is bound by constraints in a way that what we might call _creative_ design is not. And the history of computer networking shows this: the decades of networking research that led to the Internet were immensely creative, collaborative, and iterative from an engineering perspective, but they all largely stayed within the bounds of the future that Licklider had envisioned, rather than reimagining the vision, as a designer would do. It wasn't until many decades later did design resurface, with new visions such as ubiquitous computing<weiser91>, the Internet of Things, cryptocurrency, and distributed networking.

In popular culture and in industry, CS is often seen as having elements of both design and engineering. A software developer might be equally interested in envisioning futures as they might be in implementing that future. And we see blending in the archetype of the technical entrepreneur/CEO: people imagine that Mark Zuckerberg both imagined and created Facebook, that Elon Musk both imagined and engineered Tesla cars, that Steve Jobs both imagined and created the iPhone. But what is lost in these myths is the historical reality that entrepreneurs are often not doing design _or_ engineering, aside from building early prototypes, if that: 
...
Let’s consider three ways of framing CS in relation to design and their pedagogical tradeoffs. We’ll then present an example unit sketch for the third alternative, which we believe best balances disciplinary authenticity and engagement.
...
Teaching CS solely as engineering also obscures the kind of _engineering_ design considerations that nevertheless arise in programming. Deciding how code should be structured or how best to implement high-level requirements both involve some design work, because they ask students to consider tradeoffs, brainstorm, and come up with creative solutions that fit their contexts<oleson21>. And these engineering design decisions can actually introduce opportunities for biases and inequities to be embedded into software -- especially if the software operates on data about people, or if it contains an algorithm that determines resource access or allocation. Treating code-level design decisions as _purely_ engineering problems risks ignoring the broader social contexts in which software operates.
...
2. Students will be able to distinguish between distinct activities in design and engineering such as ideation, evaluation, planning, implementation, verification, and deployment.
